...
from app.services.sql import get_applications_from_date, get_all_vacancies
from app.models import Application, Vacancy
from collections import defaultdict
from app.consts import SOURCE_MAP


async def process_sourcing():

    dt_end = datetime.combine(date.today(), time.min)
    dt_start = dt_end - timedelta(days=28)
    applications = await get_applications_from_date(date_start=dt_start, date_end=dt_end)
    ...
    first_week = application_objects[0].created_at.isocalendar()[1]
    for application in application_objects:
        tmp = result.get(application.created_at.isocalendar()[1] - first_week, defaultdict(list))
        sub_dict = tmp.get(application.vacancy, {})
        if application.candidate_source is None:
            continue
        sub_dict[SOURCE_MAP[application.candidate_source]] = sub_dict.get(
            SOURCE_MAP[application.candidate_source], 0
        ) + 1
        sub_dict['_applications'] = sub_dict.get('_applications', []) + [application]
        tmp[application.vacancy] = sub_dict
        result[application.created_at.isocalendar()[1] - first_week] = tmp
